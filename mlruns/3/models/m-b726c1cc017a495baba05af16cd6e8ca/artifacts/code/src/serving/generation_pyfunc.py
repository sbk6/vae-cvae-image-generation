...
def build_artifacts(registry_path: str) -> dict:
    ...
    # MLflow copie les artefacts par nom de fichier : comme chaque dataset a
    # un `best_checkpoint.pth` et une config nommés pareil, on les recopie
    # d'abord dans un dossier de préparation avec des noms uniques (préfixés
    # par la clé de l'artefact), pour éviter qu'un fichier n'en écrase un autre.
    staging_dir = Path("reports/experiments/_deployment_staging")
    staging_dir.mkdir(parents=True, exist_ok=True)

    def stage(source: Path, key: str) -> str:
        destination = staging_dir / f"{key}{source.suffix}"
    ...
    for name, entry in registry.items():
        ...
        if cvae_entry and cvae_ckpt.exists() and cvae_cfg.exists():
            artifacts[f"cvae_checkpoint_{name}"] = stage(cvae_ckpt, f"cvae_checkpoint_{name}")
            artifacts[f"cvae_config_{name}"] = stage(cvae_cfg, f"cvae_config_{name}")
        ...
        if vae_entry and vae_ckpt.exists() and vae_cfg.exists() and centroids.exists():
            artifacts[f"vae_checkpoint_{name}"] = stage(vae_ckpt, f"vae_checkpoint_{name}")
            artifacts[f"vae_config_{name}"] = stage(vae_cfg, f"vae_config_{name}")
            artifacts[f"latent_centroids_{name}"] = stage(centroids, f"latent_centroids_{name}")
